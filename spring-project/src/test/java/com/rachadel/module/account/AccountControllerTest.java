...
//	private MockMvc mockMvc;
	...
	public void saveAccountWhenIncurrectEmailShouldReturnStatus400() {
		Account account = new Account(1L, "Manoel Rachadel Neto", "sem e-mail", "080.381.469-05");
		BDDMockito.when(accountRepository.save(account)).thenReturn(account);		
		ResponseEntity<String> response2 = restTemplate.postForEntity("/v1/accounts", account, String.class);
	...
	public void saveAccountWhenNullDocumentNumberShouldReturnStatus400() {
		Account account = new Account(1L, "Manoel Rachadel Neto", "manoelch13@gmail.com", "");
		BDDMockito.when(accountRepository.save(account)).thenReturn(account);
		ResponseEntity<String> response = restTemplate.postForEntity("/v1/accounts", account, String.class);
	...
	public void saveAccountWhenIncorrectDocumentNumberShouldReturnStatus400() {
		Account account = new Account(1L, "Manoel Rachadel Neto", "manoelch13@gmail.com", "280.381.369-08");
		BDDMockito.when(accountRepository.save(account)).thenReturn(account);
		ResponseEntity<String> response = restTemplate.postForEntity("/v1/accounts", account, String.class);
		assertThat(response.getStatusCodeValue()).isEqualTo(400);
	}		
	...
	public void listAccountsWhenCorrectShouldReturnStatusCode200() {
		List<Account> accounts = asList(new Account(1L, "Manoel Rachadel Neto", "manoelch13@gmail.com", "080.381.469-05"),
										new Account(2L, "Maria Joaquina", "maria.joaquina@gmail.com", "187.068.858-96"));
